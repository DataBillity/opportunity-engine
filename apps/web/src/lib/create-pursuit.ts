...
import { PursuitIngestResult, type PursuitIngestResult as IngestResult } from "@opportunity-engine/contracts";
...
export function recLabel(rec: Pursuit["rec"]): string {
  return { go: "Go", nogo: "No-Go", cond: "Go with conditions", pending: "Pending" }[rec] ?? rec;
}
...
export function applyIngestToPursuit(
  ...
  return {
    ...
    decisionRecord: {
      ...
      type: "D4 — Go/No-Go triage (uploaded solicitation)",
